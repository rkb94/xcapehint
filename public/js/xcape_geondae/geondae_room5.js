...
document.addEventListener("DOMContentLoaded", function () { // 페이지가 Refresh 될 때 main에서 시간 초기화
    ...
    socket.emit('reset clock', '5', group);
});
...
socket.on('restart clock', function(data){
    ...
    if(data == 'room5'){
        ...
        output5Mil *= 1;
        console.log(output5Min + ' : ' + output5Sec + ' : ' + output5Mil);
...
socket.on('active room', function(data){
    if(data == 'room5'){
        console.log('active room5 clock!!!');
        if(document.getElementById('clock').style.display == "none"){
            activeStart();
        } else {
            socket.emit('already started', 'room5', group);
...
socket.on('if started', function(group){
    if(started == true){
        ...
        socket.emit('before started', 'room5', min, sec, group);
    }
});
