...
document.addEventListener("DOMContentLoaded", function () { // 페이지가 Refresh 될 때 main에서 시간 초기화
    ...
    socket.emit('reset clock', '3', group);
});
...
socket.on('restart clock', function(data){
    ...
    if(data == 'room3'){
        console.log('restart clock room3');
        ...
        output3Mil *= 1;
        console.log(output3Min + ' : ' + output3Sec + ' : ' + output3Mil);
...
socket.on('active room', function(data){
    if(data == 'room3'){
        console.log('active room3 clock!!!');
...
socket.on('if started', function(group){
    if(started == true){
        ...
        socket.emit('before started', 'room3', min, sec, group);
    }
});
